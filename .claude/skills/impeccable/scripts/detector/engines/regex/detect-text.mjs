...
const REGEX_MATCHERS = [
  ...
  // --- Overused font ---
  {
    id: "overused-font",
    regex:
      /font-family\s*:\s*['"]?(Inter|Roboto|Open Sans|Lato|Montserrat|Arial|Helvetica|Fraunces|Geist Sans|Geist Mono|Geist|Mona Sans|Plus Jakarta Sans|Space Grotesk|Recoleta|Instrument Sans)\b/gi,
    test: () => true,
    fmt: (m) => m[0],
  },
  {
    id: "overused-font",
    regex:
      /fonts\.googleapis\.com\/css2?\?family=(Inter|Roboto|Open\+Sans|Lato|Montserrat|Fraunces|Plus\+Jakarta\+Sans|Space\+Grotesk|Instrument\+Sans|Mona\+Sans|Geist)\b/gi,
    test: () => true,
  ...
  {
    id: "pure-black-white",
    regex: /background(?:-color)?\s*:\s*(#000000|#000|rgb\(0,\s*0,\s*0\))\b/gi,
    test: () => true,
    fmt: (m) => m[0],
  },
  // --- Gradient text ---
...
const REGEX_ANALYZERS = [
  ...
  (content, filePath) => {
    ...
    for (const line of lines) {
      if (
        /<(?:h[1-6]|p|div|li|button)\b[^>]*>/i.test(line) &&
        line.trim().length > 20
      ) {
        total++;
        if (
          /text-align\s*:\s*center/i.test(line) ||
          /\btext-center\b/.test(line)
        )
          centered++;
      }
    }
