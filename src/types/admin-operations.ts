...
export type BlackoutScope =
  "VENUE_WIDE" | "COURT_SPECIFIC" | "COURT_AREA_SPECIFIC";
...
export interface CourtAreaResponse {
  id: number;
  courtId: number;
...
export interface CourtLayoutResponse {
  courtId: number;
  divisionLayout: AdminCourtDivisionLayout;
  areas: CourtAreaResponse[];
}
export interface CourtRuleRequest {
  title: string;
  description: string;
}
export interface CourtRuleResponse extends CourtRuleRequest {
  id: number;
}
...
export interface CreateCourtSportRequest {
  sportType: string;
  courtAreaCode?: string;
  capacity?: number;
  sessionDurationMinutes: number;
  startIntervalMinutes?: number;
  bookingOptions?: BookingOptionRequest[];
}
export type UpdateCourtSportRequest = Omit<
  CreateCourtSportRequest,
  "sportType" | "courtAreaCode"
>;
...
export interface PricingRuleRequest {
  ...
  effectiveTo?: string;
  optionPrices?: Array<{ bookingOptionId: number; priceAmount: number }>;
}
export interface UpdatePricingRuleRequest extends Omit<
  PricingRuleRequest,
  "endsNextDay" | "optionPrices"
> {
  priceAmount: number;
}
export type BasePricingRequest = Omit<PricingRuleRequest, "dayOfWeek">;
export interface PricingRuleResponse extends Omit<
  PricingRuleRequest,
  "priceAmount"
> {
  id: number;
  ...
  pricingRuleGroupId?: number | null;
  priceAmount: number;
  timeZoneId: string;
  active: boolean;
}
export type VenueDiscountKind = "PERCENT_OFF" | "FIXED_AMOUNT_OFF";
export interface VenueDiscountRequest {
  titleEn?: string;
  titleAr?: string;
...
export interface VenueDiscountResponse extends VenueDiscountRequest {
  id: number;
  venueId: number;
  active: boolean;
  currentlyValid: boolean;
  courts: Array<{ id: number; nameEn?: string; nameAr?: string }>;
}
export interface VenuePromotionResponse {
  id: number;
  code: string;
  kind: string;
  percentOff?: number;
