...
export function AdminPasswordResetDialog({
  ...
  compact?: boolean;
  onReset?: () => void;
  /** Supply with onOpenChange to control the dialog and hide its trigger. */
  open?: boolean;
  ...
  async function copy() {
    ...
    try {
      ...
      if (mounted.current && credential.current === current)
        setCopyStatus("Copied");
    } catch {
      if (mounted.current && credential.current === current)
        setCopyStatus("Could not copy. Select and copy the password manually.");
    }
  }
  ...
  return (
    <Dialog
      open={open}
      onOpenChange={(next) => (next ? changeOpen(true) : close())}
    >
      {!isControlled && (
        <DialogTrigger
          render={
            <Button
              ...
              title={compact ? "Reset password" : undefined}
            />
          }
        >
      ...
      <DialogContent
        ...
        className="border border-[var(--border)] bg-[var(--bg-1)] sm:max-w-lg"
      >
        ...
        {result && (
          <div className="space-y-3">
            <p className="text-sm text-[var(--text-3)]">
              Use this password only if the account holder cannot access their
              email. It cannot be viewed again after closing.
            </p>
            ...
            <div className="flex flex-wrap items-center gap-2 rounded-md border border-[var(--border)] bg-[var(--bg-0)] p-3">
              <code className="min-w-0 flex-1 select-all break-all font-mono text-lg">
