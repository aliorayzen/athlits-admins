...
export function VenueOperations({
    ...
    Promise.all([
      ...
      (error: unknown) => {
        ...
        setLoadError(
          getApiErrorMessage(
            error,
            "Venue scheduling data could not be loaded.",
          ),
        );
        setLoadState("error");
      },
  ...
  if (loadState === "error") {
    return (
      <div
        role="alert"
        className="rounded-xl border border-[var(--border)] bg-[var(--bg-1)] p-5"
      >
        <p className="text-sm text-[var(--red-text)]">{loadError}</p>
...
function CourtLimitSection({
  ...
  const [draft, setDraft] = useState(
    String(venue.courtLimit ?? Math.max(1, currentCourtCount)),
  );
  ...
  async function save() {
    ...
    if (
      !Number.isInteger(courtLimit) ||
      courtLimit < Math.max(1, currentCourtCount)
    ) {
      setError(
        `Court limit must be at least ${Math.max(1, currentCourtCount)}.`,
      );
      return;
    }
    setIsSaving(true);
  ...
  return (
    <section className="rounded-2xl border border-[var(--border)] bg-[var(--bg-1)] p-5">
      ...
      <h2 className="mt-2 text-[15px] font-semibold text-[var(--text-1)]">
        Court limit
      </h2>
      <p className="mt-1 max-w-[60ch] text-xs leading-5 text-[var(--text-3)]">
        Controls how many courts can be registered for this venue. It cannot be
        lower than the {currentCourtCount} existing courts.
      </p>
      <div className="mt-5 flex items-end gap-2">
        ...
        <Button
          disabled={isSaving || Number(draft) === venue.courtLimit}
          onClick={() => void save()}
        >
    ...
    <section className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--bg-1)] lg:col-span-2">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--border)] px-5 py-4">
        <div className="flex items-center gap-3">
          <CalendarOff className="h-4 w-4 text-[var(--amber-text)]" />
          <div>
            <h2 className="text-[15px] font-semibold text-[var(--text-1)]">
              Blackout dates
            </h2>
            <p className="text-xs text-[var(--text-4)]">
              Review booking impact before closing availability.
            </p>
          </div>
        </div>
      ...
      {blackouts.length === 0 ? (
        <p className="px-5 py-10 text-center text-sm text-[var(--text-4)]">
          No blackout dates are scheduled.
        </p>
      ) : (
...
function CreateBlackoutDialog({
  ...
  const [checkedRequest, setCheckedRequest] =
    useState<CreateBlackoutRequest | null>(null);
  ...
  function payload(): CreateBlackoutRequest | null {
    ...
    if (Boolean(draft.startTime) !== Boolean(draft.endTime)) {
      setError(
        "Provide both a start and end time, or leave both blank for all day.",
      );
      return null;
    }
    ...
    return {
      ...
      ...(draft.startTime
        ? { startTime: `${draft.startTime}:00`, endTime: `${draft.endTime}:00` }
        : {}),
  ...
  async function checkImpact() {
    ...
    try {
      ...
      setError(
        getApiErrorMessage(caught, "Blackout impact could not be checked."),
      );
    } finally {
      setIsPending(false);
    }
  ...
  async function confirm() {
    ...
    try {
      ...
      onCreated();
      toast.success(
        `Blackout confirmed; ${result.cancelledBookingCount} bookings cancelled`,
      );
    } catch (caught: unknown) {
    ...
    >
      ...
      <DialogContent className="border-[var(--border)] bg-[var(--bg-1)] sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Check the impact first. Confirmation is enabled only for the exact
        ...
        <div className="grid gap-4 py-2 sm:grid-cols-2">
          ...
          <Field label="Scope">
            <select
              value={draft.courtId}
              onChange={(event) => updateDraft({ courtId: event.target.value })}
              className={`h-9 w-full rounded-md border px-3 text-sm ${FIELD_CLASS}`}
            >
          ...
          <div className="space-y-1.5 sm:col-span-2">
            ...
            <Input
              ...
              placeholder="Maintenance, event, resurfacing..."
              className={FIELD_CLASS}
            />
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          {!checkedRequest ? (
            ...
            <Button
              ...
              className="bg-[var(--semantic-amber)] text-[var(--bg-0)] hover:brightness-110"
            >
              {isPending && <Loader2 className="h-4 w-4 animate-spin" />}
...
function BlackoutRow({
  ...
  const courtName = blackout.courtId
    ? (venue.courts.find(
        (court) => String(court.id) === String(blackout.courtId),
      )?.name ?? `Court ${blackout.courtId}`)
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-4 px-5 py-3">
      <div>
        ...
        {error && (
          <p role="alert" className="mt-1 text-xs text-[var(--red-text)]">
            {error}
          </p>
        )}
      </div>
      <AlertDialog>
        <AlertDialogTrigger
          render={
            <Button
              size="icon"
              variant="ghost"
              aria-label={`Delete blackout on ${blackout.date}`}
            />
          }
        >
        ...
        <AlertDialogContent className="border-[var(--border)] bg-[var(--bg-1)]">
          <AlertDialogHeader>
            <AlertDialogTitle>Delete this blackout?</AlertDialogTitle>
            <AlertDialogDescription>
              Availability will reopen for {blackout.date}. Cancelled bookings
          ...
          <AlertDialogFooter>
            ...
            >
              {isDeleting ? "Deleting..." : "Delete blackout"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
