...
export function CourtAvailabilityPanel({
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      (caught: unknown) => {
        if (controller.signal.aborted) return;
        setError(
          getApiErrorMessage(caught, "Court availability could not be loaded."),
        );
        setIsLoading(false);
      },
    );
  ...
  async function save() {
    ...
    if (
      mode === "CUSTOM_HOURS" &&
      (days.length === 0 || availabilityDaysWithErrors(days).length > 0)
    ) {
    ...
    try {
      ...
      try {
        ...
        toast.warning(
          "Availability was saved, but the court record could not be refreshed.",
        );
      }
    } catch (caught: unknown) {
      setError(
        getApiErrorMessage(caught, "Court availability could not be updated."),
      );
    } finally {
      setIsSaving(false);
    }
  ...
  const isDirty =
    mode !== courtSchedule.scheduleMode ||
    ...
    <section className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--bg-1)]">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--border)] px-5 py-4">
        <div>
          ...
          <p className="text-xs text-[var(--text-4)]">
            {timeZoneId ?? "Timezone not reported"} · venue defaults stay
            distinct from overrides
          </p>
        </div>
        <Button
          disabled={!isDirty || isSaving || aggregateDirty}
          onClick={() => void save()}
        >
          {isSaving ? (
      ...
      <div className="space-y-5 p-5">
        {aggregateDirty && (
          <p
            role="status"
            className="rounded-lg border border-[rgb(var(--amber-rgb)/0.25)] bg-[var(--semantic-amber-subtle)] px-4 py-3 text-xs leading-5 text-[var(--text-2)]"
          >
            Save or discard the unsaved court-detail changes before saving
            availability. This keeps the full-record editor from overwriting the
            schedule.
          </p>
        )}
        <div
          role="radiogroup"
          aria-label="Court schedule source"
          ...
          {(
            [
              [
                "VENUE_HOURS",
                "Use venue hours",
                "Inherits changes made to the venue schedule.",
              ],
              [
                "CUSTOM_HOURS",
                "Custom hours",
                "Overrides the venue defaults for this court.",
              ],
            ] as const
            ...
            <button
              ...
              className={`rounded-lg border p-3 text-left ${mode === value ? "border-[rgb(var(--teal-rgb)/0.35)] bg-[var(--teal-subtle)]" : "border-[var(--border)] bg-[var(--bg-0)] hover:border-[var(--border-strong)]"}`}
            >
        ...
        {mode === "CUSTOM_HOURS" ? (
          ...
          <p className="rounded-lg bg-[var(--bg-0)] px-4 py-3 text-xs leading-5 text-[var(--text-3)]">
            This court currently follows the venue’s{" "}
            {decodeVenueAvailabilityDays(venueSchedule.days).length} open days.
          </p>
        )}
        {error && (
          <p role="alert" className="text-sm text-[var(--red-text)]">
...
function CourtConflicts({
  ...
  async function check() {
    ...
    try {
      setResult(
        await getCourtConflicts(venueId, courtId, { startTime, endTime }),
      );
    } catch (caught: unknown) {
  ...
  return (
    <div className="border-t border-[var(--border)] pt-5">
      <div className="flex items-center gap-2">
        <CalendarSearch className="h-4 w-4 text-[var(--text-3)]" />
        <h3 className="text-sm font-semibold text-[var(--text-1)]">
          Booking conflicts
        </h3>
      </div>
      <p className="mt-1 text-xs text-[var(--text-4)]">
        Check a local date-time window before changing hours or destructive
        configuration.
      </p>
      ...
      {result && (
        <div
          className={`mt-3 rounded-lg border px-4 py-3 ${result.conflict ? "border-[rgb(var(--amber-rgb)/0.25)] bg-[var(--semantic-amber-subtle)]" : "border-[var(--border)] bg-[var(--bg-0)]"}`}
        >
          <p className="flex items-center gap-2 text-sm font-medium text-[var(--text-1)]">
            {result.conflict && (
              <AlertTriangle className="h-4 w-4 text-[var(--amber-text)]" />
            )}
            {result.conflict
              ? `${result.conflictingBookingIds.length} conflicting bookings`
              : "No booking conflicts"}
          </p>
          {result.conflict && (
            <p className="mt-1 font-mono text-xs text-[var(--text-3)]">
              IDs: {result.conflictingBookingIds.join(", ")} ·{" "}
              <Link
                href="/dashboard/bookings"
                className="text-[var(--teal-text)] hover:underline"
              >
                Open bookings
              </Link>
            </p>
