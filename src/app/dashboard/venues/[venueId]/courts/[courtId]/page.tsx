...
export default function CourtDetailPage() {
  ...
  async function loadCourt() {
    setIsLoading(true);
    try {
      const [courtRecord, venue] = await Promise.all([
        getCourt(venueId, courtId),
        getVenue(venueId),
      ]);
  ...
  async function save() {
    ...
    try {
      ...
      if (getApiErrorStatus(error) === 409) {
        ...
        toast.error(
          "Someone else updated this court. Reload before saving again.",
        );
      } else {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-5 pb-12">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="font-mono text-xs text-[var(--text-4)]">
              Court {courtId} · version {record.version}
            </p>
          </div>
        </div>
        <div className="flex gap-2">
          {record.active && venueActive && (
            <Link
              href={`/dashboard/venues/${venueId}/courts/${courtId}/bookings/new`}
            >
          ...
          <SaveButton
            disabled={!isDirty || isSaving || hasVersionConflict}
            busy={isSaving}
            onClick={save}
          />
        </div>
      ...
      {hasVersionConflict && (
        <div
          role="alert"
          className="flex flex-col gap-3 rounded-xl border border-[rgb(var(--amber-rgb)/0.25)] bg-[var(--semantic-amber-subtle)] px-4 py-3 sm:flex-row sm:items-center sm:justify-between"
        >
          <div>
            <p className="text-sm font-semibold text-[var(--amber-text)]">
              This court changed after you loaded it
            </p>
            <p className="text-xs text-[var(--text-3)]">
              Reload the latest version, then reapply your changes. Your stale
              version was not saved.
            </p>
          </div>
          <Button variant="outline" onClick={loadCourt}>
      ...
      <Section
        number="01"
        title="Court details"
        description="The complete record is replaced atomically when saved."
      >
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <Field label="English name">
            <Input
              required
              value={record.nameEn}
              onChange={(e) => updateRecord({ nameEn: e.target.value })}
          ...
          <Field label="Arabic name">
            <Input
              ...
              dir="rtl"
              value={record.nameAr}
              onChange={(e) => updateRecord({ nameAr: e.target.value })}
              className={FIELD_CLASS}
            />
          </Field>
          <Field label="Dimensions">
            <Input
              value={record.dimensionsLabel ?? ""}
              onChange={(e) =>
                updateRecord({ dimensionsLabel: e.target.value })
              }
              placeholder="40 x 20m"
              className={FIELD_CLASS}
            />
          </Field>
          <Field label="Surface">
            <select
              value={record.surfaceType}
              onChange={(e) =>
                updateRecord({ surfaceType: e.target.value as SurfaceType })
              }
          ...
          <Field label="Environment">
            <select
              ...
              onChange={(e) =>
                updateRecord({
                  environment: e.target.value as CourtEnvironment,
                })
              }
              className={SELECT_CLASS}
            >
          ...
          <Field label="Cancellation policy">
            <Input
              readOnly
              value={
                record.cancellationPolicyId
                  ? `Assigned policy ${record.cancellationPolicyId}`
                  : "No policy assigned"
              }
              className={FIELD_CLASS}
            />
          ...
          <Field label="Booking status">
            <select
              ...
              onChange={(e) =>
                updateRecord({ active: e.target.value === "ACTIVE" })
              }
              className={SELECT_CLASS}
            >
      ...
      <Section
        number="02"
        title="Amenities"
        description="Only backend-supported amenities can be attached."
      >
        <div className="flex flex-wrap gap-2">
          {AMENITIES.map(({ value, label }) => {
            const selected = record.amenityIds.includes(value);
            return (
              <button
                key={value}
                type="button"
                ...
                onClick={() =>
                  updateRecord({
                    amenityIds: selected
                      ? record.amenityIds.filter((id) => id !== value)
                      : [...record.amenityIds, value],
                  })
                }
                className={`rounded-lg border px-3 py-2 text-sm transition-colors ${selected ? "border-[rgb(var(--teal-rgb)/0.3)] bg-[var(--teal-subtle)] text-[var(--teal-text)]" : "border-[var(--border)] bg-[var(--bg-0)] text-[var(--text-3)] hover:border-[var(--border-strong)]"}`}
              >
                {label}
              </button>
            );
      ...
      <Section
        number="03"
        title="Image order"
        description="The first ID is primary. Saving replaces the complete ordered list; maximum 5."
      >
        <div className="space-y-2">
          {record.imageIds.map((imageId, index) => (
            <div
              key={imageId}
              className="flex items-center justify-between rounded-lg border border-[var(--border)] bg-[var(--bg-0)] px-3 py-2"
            >
              <div>
                ...
                {index === 0 && (
                  <span className="ml-2 text-[10px] font-semibold uppercase text-[var(--teal-text)]">
                    Primary
                  </span>
                )}
              </div>
              <div className="flex gap-1">
                <Button
                  ...
                  aria-label={`Move image ${imageId} up`}
                >
                ...
                <Button
                  ...
                  size="icon"
                  variant="ghost"
                  disabled={index === record.imageIds.length - 1}
                  onClick={() => moveImage(index, 1)}
                  aria-label={`Move image ${imageId} down`}
                >
                ...
                <Button
                  ...
                  onClick={() =>
                    updateRecord({
                      imageIds: record.imageIds.filter((id) => id !== imageId),
                    })
                  }
          ...
          {record.imageIds.length === 0 && (
            <p className="rounded-lg border border-dashed border-[var(--border)] py-8 text-center text-sm text-[var(--text-4)]">
              No court images
            </p>
          )}
          <label
            htmlFor={imageInputId}
            className={`mt-3 inline-flex h-9 cursor-pointer items-center rounded-lg border border-[var(--border-strong)] bg-[var(--bg-2)] px-3 text-sm font-medium text-[var(--text-2)] hover:bg-[var(--bg-3)] ${record.imageIds.length >= 5 ? "pointer-events-none opacity-50" : ""}`}
          >
          ...
          <input
            id={imageInputId}
            disabled={isSaving || record.imageIds.length >= 5}
            type="file"
            accept="image/*"
            multiple
            className="sr-only"
      ...
      <section className="overflow-hidden rounded-2xl border border-[rgb(var(--red-rgb)/0.22)] bg-[var(--bg-1)]">
        <header className="border-b border-[rgb(var(--red-rgb)/0.16)] px-5 py-4">
          <h2 className="text-[15px] font-semibold text-[var(--red-text)]">
            Danger zone
          </h2>
          <p className="text-xs text-[var(--text-4)]">
            Deleting a court is permanent and may be rejected when bookings
            depend on it.
          </p>
        </header>
        <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-[var(--text-3)]">
              Backend conflicts are shown here so dependent records can be
              resolved safely.
            </p>
          </div>
          <Dialog
          ...
          >
            <DialogTrigger
              render={
                <Button
                  variant="outline"
                  className="border-[rgb(var(--red-rgb)/0.3)] text-[var(--red-text)] hover:bg-[var(--semantic-red-subtle)]"
                />
              }
            >
            ...
            <DialogContent className="border-[var(--border)] bg-[var(--bg-1)]">
              <DialogHeader>
                <DialogTitle>
                  Delete {record.nameEn || record.nameAr}?
                </DialogTitle>
                <DialogDescription>
                  Type the court name exactly to confirm. This action cannot be
                  undone.
                </DialogDescription>
              </DialogHeader>
...
function Section({
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--bg-1)]">
      <header className="flex gap-3 border-b border-[var(--border)] px-5 py-4">
        <span className="font-mono text-xs text-[var(--teal-text)]">
          {number}
        </span>
        <div>
...
function StatusBadge({ active }: { active: boolean }) {
  return (
    <span
      className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase ${active ? "bg-[var(--semantic-green-subtle)] text-[var(--green-text)]" : "bg-[var(--bg-2)] text-[var(--text-4)]"}`}
    >
