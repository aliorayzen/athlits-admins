...
export default function CreateCourtPage() {
  ...
  const [divisionLayout, setDivisionLayout] =
    useState<CourtDivisionLayout>("FULL");
  ...
  function updateSport(index: number, patch: Partial<SportDraft>) {
    setSports((current) =>
      current.map((sport, i) => (i === index ? { ...sport, ...patch } : sport)),
    );
  }
  ...
  function updatePricing(index: number, patch: Partial<SportDraft["pricing"]>) {
    setSports((current) =>
      current.map((sport, i) =>
        i === index
          ? { ...sport, pricing: { ...sport.pricing, ...patch } }
          : sport,
      ),
    );
  }
  ...
  function updateEquipment(
    ...
    patch: Partial<CourtEquipmentRequest>,
  ) {
    updateSport(sportIndex, {
      equipment: sports[sportIndex].equipment.map((item, i) =>
        i === equipmentIndex ? { ...item, ...patch } : item,
      ),
    });
  }
    ...
    try {
      const payload: CreateCourtRequest = {
        ...
        sports: sports.map((sport) => ({
          ...
          equipment: sport.equipment.map((item) => ({
            ...item,
      ...
      if (images.length > 0) {
        try {
          ...
          toast.warning(
            `Court created, but images were not uploaded: ${getApiErrorMessage(error)}`,
          );
  ...
  return (
    <form onSubmit={handleSubmit} className="mx-auto max-w-5xl space-y-5 pb-12">
      <header className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-[var(--text-3)]">
            {venueName || "Venue"} · created active and bookable
          </p>
        </div>
      </header>
      ...
      <FormSection
        number="01"
        title="Court details"
        description="Identity, surface, environment, and how the playing area divides."
      >
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Environment">
            <select
              value={environment}
              onChange={(e) =>
                setEnvironment(e.target.value as CourtEnvironment)
              }
              className={SELECT_CLASS}
            >
          ...
          <Field label="Division layout">
            <select
              value={divisionLayout}
              onChange={(e) =>
                setDivisionLayout(e.target.value as CourtDivisionLayout)
              }
              className={SELECT_CLASS}
            >
              <option>FULL</option>
              <option>HALVES</option>
              <option>THIRDS</option>
              <option>QUARTERS</option>
            </select>
          </Field>
          <Field label="Venue currency">
            <Input
              readOnly
              value={currencyCode}
      ...
      <FormSection
        number="02"
        title="Sports and pricing"
        description="Prices are stored in the smallest currency unit per hour. Add each playable configuration."
      >
        <div className="space-y-4">
          {sports.map((sport, index) => (
            <div
              key={index}
              className="rounded-xl border border-[var(--border)] bg-[var(--bg-0)] p-4"
            >
              <div className="mb-4 flex items-center justify-between">
                ...
                {sports.length > 1 && (
                  <Button
                    ...
                    variant="ghost"
                    onClick={() =>
                      setSports((items) => items.filter((_, i) => i !== index))
                    }
                  >
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <Field label="Sport type" required>
                  <Input
                    ...
                    onChange={(e) =>
                      updateSport(index, { sportType: e.target.value })
                    }
                    className={FIELD_CLASS}
                  />
                </Field>
                <Field label="Area code" required>
                  <Input
                    required
                    value={sport.courtAreaCode}
                    onChange={(e) =>
                      updateSport(index, { courtAreaCode: e.target.value })
                    }
                    className={FIELD_CLASS}
                  />
                </Field>
                <Field label="Capacity" required>
                  <Input
                    ...
                    onChange={(e) =>
                      updateSport(index, { capacity: Number(e.target.value) })
                    }
                    className={FIELD_CLASS}
                  />
                </Field>
                <Field label="Session minutes" required>
                  <Input
                    required
                    min={1}
                    type="number"
                    value={sport.sessionDurationMinutes}
                    onChange={(e) =>
                      updateSport(index, {
                ...
                <Field
                  label={`Off-peak (${currencyCode || "currency"} cents/hour)`}
                  required
                >
                  <Input
                    required
                    min={0}
                    type="number"
                    value={sport.pricing.offPeakCentsPerHour}
                    onChange={(e) =>
                      updatePricing(index, {
                        offPeakCentsPerHour: Number(e.target.value),
                      })
                    }
                    className={FIELD_CLASS}
                  />
                ...
                <Field
                  label={`Peak (${currencyCode || "currency"} cents/hour)`}
                  required
                >
                  <Input
                    required
                    min={0}
                    type="number"
                    value={sport.pricing.peakCentsPerHour}
                    onChange={(e) =>
                      updatePricing(index, {
                        peakCentsPerHour: Number(e.target.value),
                      })
                    }
                    className={FIELD_CLASS}
                  />
                ...
                <Field label="Peak starts">
                  <Input
                    type="time"
                    value={minutesToTime(
                      sport.pricing.peakWindow?.startMinutes ?? 1080,
                    )}
                    onChange={(e) =>
                      updatePricing(index, {
                        peakWindow: {
                          ...
                          endMinutes:
                            sport.pricing.peakWindow?.endMinutes ?? 1320,
                        },
                      })
                    }
                    className={FIELD_CLASS}
              ...
              <div className="mt-5 space-y-3">
                ...
                {sport.equipment.map((item, equipmentIndex) => (
                  <div
                    key={equipmentIndex}
                  ...
                  >
                    <Input
                      ...
                      value={item.nameEn}
                      onChange={(e) =>
                        updateEquipment(index, equipmentIndex, {
                    ...
                    />
                    <Input
                      aria-label="Reservation limit"
                      min={0}
                      type="number"
                      value={item.perReservationLimit}
                      onChange={(e) =>
                        updateEquipment(index, equipmentIndex, {
                          perReservationLimit: Number(e.target.value),
                        })
                      }
                      className={FIELD_CLASS}
                    />
                    <Button
                      ...
                      onClick={() =>
                        updateSport(index, {
                          equipment: sport.equipment.filter(
          ...
          <Button
            type="button"
            variant="outline"
            onClick={() => setSports((items) => [...items, emptySport()])}
          >
      ...
      <FormSection
        ...
        description="Amenities are restricted to supported options. The first uploaded image becomes primary."
      >
        <div className="flex flex-wrap gap-2">
          {AMENITIES.map(({ value, label }) => (
            <button
              key={value}
              type="button"
              aria-pressed={amenityIds.includes(value)}
              onClick={() => toggleAmenity(value)}
              className={`rounded-lg border px-3 py-2 text-sm transition-colors ${amenityIds.includes(value) ? "border-[rgb(var(--teal-rgb)/0.3)] bg-[var(--teal-subtle)] text-[var(--teal-text)]" : "border-[var(--border)] bg-[var(--bg-0)] text-[var(--text-3)] hover:border-[var(--border-strong)]"}`}
            >
              {label}
            </button>
          ))}
        ...
        <div className="mt-5">
          <Label
            ...
            className="mb-2 block text-xs text-[var(--text-3)]"
          >
            Court images
          </Label>
          <label
            htmlFor={imageInputId}
            className="flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-dashed border-[var(--border-strong)] bg-[var(--bg-0)] px-4 py-8 text-sm text-[var(--text-3)] hover:bg-[var(--bg-2)]"
          >
            <ImagePlus className="h-4 w-4" />
            {images.length
              ? `${images.length} image${images.length === 1 ? "" : "s"} selected`
              : "Choose images in primary-first order"}
          </label>
          <input
            id={imageInputId}
            type="file"
